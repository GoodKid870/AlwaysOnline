...
class FriendsRepository {
    ...
    static GetDatabasefriendsListFromUser(userMail) {
    ...
    static CreateFriendObject(username, userMail, userId, avatar) {
        return {
            "friends": {
                "username": username,
                "usermail": userMail,
    ...
    static AddFriendToList(friendList, userId) {
        friendList.push({ "userId": userId });
    }
    //а здесь происходит непосредственно сама логика
    static AddSomeFriend(userMail, username, email, userId, avatar) {
        try {
            const user = UserRepository.GetUserFromEmail(userMail);
    ...
    static ChangeFriendsData(usermail, newusername, newusermail) {
        try {
            const user = UserRepository.GetUserFromEmail(usermail);
    ...
    static ChangeFriendAvatar(usermail, username, avatar) {
        try {
            const user = UserRepository.GetUserFromEmail(usermail);
            const friend = FriendsRepository.GetDatabasefriends(user.userInfo.userId);
            if (friend != undefined) {
    ...
    static HasFriend(usermail, userId) {
        const user = UserRepository.GetUserFromEmail(usermail);
        try {
    ...
    static DeleteSomeFriend(usermail, userId, friendMail, friendUserId) {
        const user = UserRepository.GetUserFromEmail(usermail);
