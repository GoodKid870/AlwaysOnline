...
class Database {
    ...
    static GetDatabaseObject(type, id) {
        try {
    ...
    static SetDatabaseObject(type, id, object) {
        try {
    ...
    static GetAllDatabaseUsers(currentUser) {
        try {
    ...
    static GetDatabaseMessageObject(type, id) {
        try {
            const data = fs.readFileSync(`./server/database/${type}/${id}.json`, "utf-8");
            return JSON.parse(data);
        }
    ...
    static SetDatabaseMessageObjects(type, filename, object) {
        try {
    ...
    static ChangeDatabaseUser(type, filename, newUserMail) {
        try {
